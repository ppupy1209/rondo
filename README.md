...
```sh
curl -fsSL https://raw.githubusercontent.com/ppupy1209/rondo/v0.14.3/install.sh | sh
```
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/ppupy1209/rondo/v0.14.3/install.ps1))) -Version v0.14.3
```
...
Windows에서는 원하는 에이전트 패널 안을 마우스로 클릭하면 바로 전환됩니다. 키보드로는 `Ctrl+p`를 누른 뒤 방향키를 누르세요. Rondo는 실행한 Zellij 클라이언트에서만 마우스와 `normal` 입력 모드를 활성화하며 사용자 전역 Zellij 설정은 변경하지 않습니다.
...
평소에는 세부 명령을 외울 필요가 없습니다. 에이전트 패널에서 자연스럽게 요청하면 해당 에이전트가 Rondo 작업을 대신 시작합니다.
...
일반 사용자는 에이전트에게 “이 내용을 기억해줘”라고 말하고, `shell` 탭의 `rondo` 메뉴에서 제안을 선택하면 됩니다. 아래 명령은 같은 작업을 직접 제어하거나 자동화할 때 사용합니다.
