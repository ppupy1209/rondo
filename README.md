...
- AI가 다른 AI에게 보내는 요청은 대상 패널에 사람의 입력처럼 바로 전달되며, Relay에는 복구·감사용 사본이 남습니다.
...
첫 실행에서 방향키로 이동하고 `Space`로 AI를 선택·해제한 뒤 `Enter`로 완료합니다. 이어서 승인 모드도 방향키와 `Enter`로 선택합니다. AI 이름이나 모드 이름을 직접 입력할 필요가 없습니다. 이후에는 `rondo`만 입력하면 같은 프로젝트의 Rondo 화면으로 바로 들어갑니다.

승인 모드는 두 가지입니다.

- **일일이 승인**: 수정과 명령을 사용자가 확인합니다. 새 프로젝트의 기본값입니다.
- **프로젝트 안에서 자동 승인**: 각 공식 CLI의 안전장치를 유지하면서 프로젝트 범위의 작업을 자동 처리합니다. Claude는 `auto`, Codex는 `workspace-write` 자동 검토, Gemini는 샌드박스 안의 편집 자동 승인을 사용합니다. 시스템 전체 권한을 여는 위험한 무제한 승인 옵션은 사용하지 않습니다.

설정을 바꾸려면 언제든 `rondo setup`을 다시 실행합니다. 자동화에서는 `rondo setup --agents claude,codex --approval manual|workspace`를 사용할 수 있습니다.
...
Relay는 별도 탭이며 `Ctrl+t`를 누른 뒤 방향키로 전환합니다. 다른 AI에게 요청하면 Rondo가 대상 AI 패널에 텍스트를 직접 입력하고 실행하며, Relay에는 같은 내용을 감사 기록으로 남깁니다. 하단의 Zellij 상태 표시줄에서 현재 사용할 수 있는 단축키를 확인할 수 있습니다. 세션을 유지하고 나가려면 `Ctrl+o` 다음 `d`, AI와 세션을 모두 종료하려면 `Ctrl+q`를 누릅니다. Rondo는 글꼴 호환성이 높은 단순 UI, 기본 모드 `normal`, 마우스 `on`으로 시작합니다.
...
```text
.rondo/
  config.json       활성 AI, 승인 모드, 맥락 공유, 자동 인계 설정
...
`context.md`는 활성 AI와 승인 모드, 현재 목표, 체크포인트, 인계, 독립 검증, Git 작업 트리 요약을 담는 공유 문서입니다. Rondo를 시작할 때 자동으로 생성·갱신되며 `rondo status`로 현재 설정을 확인할 수 있습니다.
