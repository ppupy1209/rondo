...
5. Claude의 활성 사용 한도 중 하나가 임계치에 닿으면 연속성 패킷을 만들거나 Codex가 자동으로 이어서 작업합니다.
...
`auto`는 setup에서 명시적으로 골라야만 켜집니다. Codex의 `workspace-write` sandbox를 사용하고 네트워크 옵션이나 승인 우회는 켜지 않습니다. push·배포·원격 변경·파괴적 작업을 금지하는 계약도 함께 전달하며, 실행 결과는 패킷 옆에 기록합니다.
